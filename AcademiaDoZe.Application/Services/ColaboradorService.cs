...
public class ColaboradorService : IColaboradorService
{
    ...
    public async Task<ColaboradorDto?> ObterPorIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            var logradouro = await _logradouroRepoFactory().ObterPorId(colaborador.Endereco.Logradouro.Id, cancellationToken);
    ...
    public async Task<IEnumerable<ColaboradorDto>> ObterTodosAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_logradouroRepoFactory != null)
        {
            var logradouroIds = colaboradores.Select(c => c.Endereco.Logradouro.Id).Distinct().ToList();
            ...
            return [.. colaboradores.Select(c => c.ToDto(logradouros.GetValueOrDefault(c.Endereco.Logradouro.Id)))];
        }
