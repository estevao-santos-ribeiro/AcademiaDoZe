...
public class AlunoService : IAlunoService
{
    ...
    public async Task<AlunoDto?> ObterPorIdAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            var logradouro = await _logradouroRepoFactory().ObterPorId(aluno.Endereco.Logradouro.Id, cancellationToken);
    ...
    public async Task<IEnumerable<AlunoDto>> ObterTodosAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_logradouroRepoFactory != null)
        {
            var logradouroIds = alunos.Select(a => a.Endereco.Logradouro.Id).Distinct().ToList();
            ...
            return [.. alunos.Select(a => a.ToDto(logradouros.GetValueOrDefault(a.Endereco.Logradouro.Id)))];
        }
